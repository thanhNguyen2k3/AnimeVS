...
import AlertModelConfirmDelete from '../ui/modal/AlertModelConfirmDelete';
...
const Playlist = ({ data }: Props) => {
    const handleSoftDelete = async (id: string, data: MoviePlaylist) => {
        try {
            const res = await instance.patch(`/v1/movie/${id}`, {
                data,
            });

            if (res.status === 200) {
                toastSuccessResponse('Đã chuyển vào thùng rác');
            }
        } catch (error) {
    ...
    const columns: GridColDef<MoviePlaylist>[] = [
        ...
        },
        {
            ...
            width: 110,
            editable: false,
            valueFormatter: (params) => formatTimeToNow(new Date(params?.value)),
        },
        {
            field: 'updatedAt',
